    ...
    if platform.system() == "Windows":
        for path in [os.environ.get('ProgramFiles'),os.environ.get('ProgramFiles(86)',None),os.environ.get('ProgramW6432',None)]:
            if path != None and os.path.isdir(path+"/eSpeak"):
                available.append(eSpeak)#"eSpeak")
    else:
        if os.path.isfile("/usr/bin/espeak") or os.path.isdir("/usr/share/espeak-data") or os.path.isdir(os.path.join(os.path.expanduser("~/"),"espeak-data")):
            available.append(eSpeak)#"eSpeak")
        ...
        if platform.system() == "Windows":
            for path in [os.environ.get('ProgramFiles'),os.environ.get('ProgramFiles(86)',None),os.environ.get('ProgramW6432',None)]:
                if path != None and os.path.isdir(path+"/freetts"):
                    available.append(FreeTTS)#"FreeTTS")
